...
1. 基于DNS的GSLB: 通过LDNS的所属区域，给用户返回该区域的IP列表，存在问题LDNS错误导致返回的IP不对）
    * 无法获取用户IP地址，只能获取到LDNS 
          2). 基于http 302的GSLB
    * 用户访问http server的时候根据IP地址，返回一个302的头信息，引导用户到最近的边缘节点 （多一次http请求 tcp3次握手）
    * 302调度一般都是先通过DNS获取到一个边缘节点IP，再进行一次302，流程太长
          3). http DNS
