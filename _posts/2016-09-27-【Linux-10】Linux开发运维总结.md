...
3. 域名解析: `$ dig www.baidu.com`
4. 查看某个端口是否被占用
   + `$ lsof -i:1234`
   + `$ netstat -apn | grep 1234`
5. Python命令格式化json数据: `$ python -mjson.tool json.dat`
6. 查看kafka server版本: `$ sudo find / -name \*kafka_\* | head -1 | grep -o '\kafka[^\n]*’`
7. Mac进入自带的docker-for-desktop节点: `$ screen ~/Library/Containers/com.docker.docker/Data/vms/0/tty`
8. 清理所有的screen窗口: `$ pkill SCREEN`
9. 计算2个文件交集和差集
