...
36. `iota并不会经常从0开始`，iota实际上表示的是当前常量在代码块里面的行编号
    ...
    ```
37. `栈和堆变量`，Golang程序我们大部分情况下都不需要考虑变量是在栈上还是在堆上分配的，Go会根据变量的大小进行分析，确认要存储到栈上还是堆上。我们可以通过 `-gcflags -m` 这个参数来确认参数分配位置
...
runtime.GOMAXPROCS(20)
    ...
    ```
39. `goroutine并行运行导致结果不符合预期`，如果通过runtime.GOMAXPROCS(x)设置并行运行的逻辑CPU核个数大于1，那么goroutine之间是可以并行运行的，
